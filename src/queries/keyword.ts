export default function keywordsearch(keyword: string) {
    const query = `
        ...
        where {
            ...
            filter (
                contains(?title, "${keyword}") || contains("${keyword}", ?title) || 
                contains(?creator, "${keyword}") || contains("${keyword}", ?creator) || 
                contains(?artform, "${keyword}") || contains("${keyword}", ?artform) || 
                contains(?loc_created, "${keyword}") || contains("${keyword}", ?loc_created)
            ) .
        }
    `.replace(/[\r\n\t]/g, " ");
