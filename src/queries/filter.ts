...
export default function filterSearch(queries: SearchQueries, useLocation: boolean) {
    ...
    const hasFilter = !!queries.artform || !!queries.location || 
                        !!queries.material || !!useLocation || !!queries.title;
    ...
    var query = 
        `PREFIX foaf: <http://xmlns.com/foaf/0.1/>
            ...
            select * { {
                ...
                (sample(float:(?geonames)) as ?geonames)
                where {`;
            ...
            var filter = ''.concat(
                ...
                queries.location ? 
                    `?object sdo:identifier ?id; 
                        sdo:locationCreated/sdo:name ?loc_created0.
                    filter(contains(lcase(str(?loc_created0)), 
                        lcase(str("${queries.location}")))) ` : '')
            query += filter
        }
    ...
    query = query.concat(`} group by 
        ?id ?object ?title ?creator ?image ?artform ?geonames 
                        }} limit 50`);
    query = query.replace(/[\r\n\t]/g, " ");
